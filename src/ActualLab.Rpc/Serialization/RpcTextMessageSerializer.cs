    ...
    : RpcMessageSerializer(peer), ITextSerializer<RpcMessage>
{
    private static readonly JsonSerializerOptions JsonSerializerOptions = new() {
        WriteIndented = false,
        TypeInfoResolver = JsonRpcMessageContext.Default,
    };
    private static readonly byte Delimiter = (byte)'\n';
    private static readonly byte[] DelimiterBytes = new[] { Delimiter };
    ...
    public override RpcMessage Read(ReadOnlyMemory<byte> data, out int readLength)
    {
        ...
        var m = (JsonRpcMessage)JsonSerializer.Deserialize(ref reader, typeof(JsonRpcMessage), JsonSerializerOptions)!;
    ...
    public override void Write(IBufferWriter<byte> bufferWriter, RpcMessage value)
    {
        ...
        JsonSerializer.Serialize(writer, new JsonRpcMessage(value), typeof(JsonRpcMessage), JsonSerializerOptions);
        writer.Flush();
