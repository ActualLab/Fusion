...
/// <summary>
/// Compact variant of <see cref="RpcByteMessageSerializerV4"/> that transmits method references as hash codes only.
/// </summary>
public class RpcByteMessageSerializerV4Compact(RpcPeer peer) : RpcByteMessageSerializerV4(peer)
{
    ...
    private void WriteWithArgumentDataNativeLittleEndian(ArrayPoolBuffer<byte> buffer, RpcOutboundMessage message, ReadOnlyMemory<byte> argumentData)
    {
        if (argumentData.Length > MaxArgumentDataSize)
            throw Errors.SizeLimitExceeded();
