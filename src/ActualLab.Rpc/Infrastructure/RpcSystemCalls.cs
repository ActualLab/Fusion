    ...
    : RpcServiceBase(services), IRpcSystemCalls, IRpcPolymorphicArgumentHandler
{
    ...
    public Task<RpcNoWait> Error(ExceptionInfo error)
    {
        ...
        // The call is looked up before ToException(), so an unmatched RelatedId never resolves
        // the peer-supplied exception type name
        var outboundCall = peer.OutboundCalls.Get(outboundCallId);
        if (outboundCall is null)
            return RpcNoWait.Tasks.Completed;

        // ToException() returns null for ExceptionInfo.None, and SetError requires a non-null exception
        var exception = error.ToException() ?? Errors.EmptyErrorResponse();
        if (exception is RpcRerouteException)
            Log.LogWarning(exception, "Got RpcRerouteException from remote peer");
        outboundCall.SetError(exception, context);
        return RpcNoWait.Tasks.Completed;
    }
    ...
    public Task<RpcNoWait> KeepAlive(long[] localIds)
    {
        var context = RpcInboundContext.GetCurrent();
        var peer = context.Peer;
        peer.SharedObjects.KeepAlive(localIds);
        return RpcNoWait.Tasks.Completed;
    }
    ...
    public Task<RpcNoWait> Ack(long nextIndex, Guid hostId = default)
    {
        ...
        if (peer.SharedObjects.Get(localId) is RpcSharedStream stream)
            stream.OnAck(nextIndex, hostId);
        else
            peer.Hub.SystemCallSender.Disconnect(peer, [localId]);
        return RpcNoWait.Tasks.Completed;
    }
    ...
    {
        var context = RpcInboundContext.GetCurrent();
        if (context.RelatedObject is RpcStream stream) // IsValidCall sets it
    ...
    public Task<RpcNoWait> End(long index, ExceptionInfo error)
    {
        ...
        var peer = context.Peer;
        var localId = context.Message.RelatedId;
        if (peer.RemoteObjects.Get(localId) is RpcStream stream)
