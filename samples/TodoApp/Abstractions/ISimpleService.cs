...
public sealed partial record Table<T>(
    ...
    [property: DataMember(Order = 1), MemoryPackOrder(1), Key(1), MemoryPackAllowSerialize] RpcStream<Row<T>> Rows);

[DataContract, MemoryPackable(GenerateType.VersionTolerant), MessagePackObject]
public sealed partial record Row<T>(
    [property: DataMember(Order = 0), MemoryPackOrder(0), Key(0)] int Index,
    [property: DataMember(Order = 1), MemoryPackOrder(1), Key(1), MemoryPackAllowSerialize] RpcStream<T> Items);
