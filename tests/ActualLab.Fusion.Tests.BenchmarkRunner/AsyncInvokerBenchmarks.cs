...
public class AsyncInvokerBenchmarks
{
    ...
    private static readonly ProxyMethodTable MethodTable = new(typeof(AsyncInvokerBenchmarks), [Method]);
    private static readonly Func<ArgumentList, Task<Unit>> Intercepted = static _ => TaskExt.UnitTask;

    private readonly Invocation _invocation = new(new object(), MethodTable, 0, ArgumentList.New(), Intercepted);
