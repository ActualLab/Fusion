...
import { ComputeFunction, ConsistencyState } from '../src/index.js';
...
describe('ComputeFunction', () => {
    ...
    it('should have a unique sequential id', () => {
        const fn1 = new ComputeFunction('alpha', function () {
            return 1;
        });
        const fn2 = new ComputeFunction('beta', function () {
            return 2;
        });
    ...
    it('should build keys with Record Separator delimiter', () => {
        const fn = new ComputeFunction('getValue', function () {
            return 1;
        });
    ...
    it('should support overridable argToString', () => {
        const fn = new ComputeFunction('custom', function () {
            return 1;
        });
    ...
    it('should cache the result for the same instance+args', async () => {
        let callCount = 0;
        const fn = new ComputeFunction('count', function () {
            return ++callCount;
        });
    ...
    it('should recompute after invalidation', async () => {
        let callCount = 0;
        const fn = new ComputeFunction('inc', function () {
            return ++callCount;
        });
