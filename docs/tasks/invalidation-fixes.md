...
- **Fable 5 follow-up, wave 2** (fixes from the full `/code-review high` of all the above, 2026-07-13): `a23320e7`+`86bbab24` gap-set cadence rework (due-gated queries, no full-batch starvation, horizon expiry for budgeted entries) · `981fb82e` error auto-invalidation routed by `Transiency` (`Terminal` → `AutoInvalidationDelay`; `NonTransient` → `min(AutoInvalidationDelay, NonTransientErrorInvalidationDelay)`) · `2fbc7d0b` local log readers woken directly in `DbLogWatcher.NotifyChanged` (fixes the Redis self-skip latency regression + the same pre-existing Npgsql gap) · `bd7602ce` non-singleton compute services with command handlers rejected at registration; replay qualification uses a guarded service resolution (no more `ValidateScopes` faults for scoped non-compute command services) · `d3a5edc7` `IState.Invalidated` is exactly-once per generation · `d44d361f` synchronous hand-off consumption invariant documented + debug-asserted · `9c47991a` at-least-once listener delivery documented · `1141ffd4` misleading operations-log discard log fixed · `c25d5da6` per-listener failure logs restored + minor cleanups.
