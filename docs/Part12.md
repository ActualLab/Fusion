...
- `ActualLab.Rpc` &ndash; a new communication library, which is way more performant and efficient than the previous communication layer (HTTP + WebSockets). It doesn't require ASP.NET Core controllers and `IXxxClientDef` interfaces to work, so it's also way easier to use it.
...
- gRPC &ndash; including its official .NET implementation
- SignalR
...
2. Above behavior should be extremely reliable &ndash; in particular, the calls must "go on" even if you're turning Airplane Mode on, getting disconnected, etc.
...
5. There must be a way to plug in ~ any binary serializers &ndash; most importantly, [MemoryPack](https://github.com/Cysharp/MemoryPack) and [MessagePack](https://github.com/MessagePack-CSharp/MessagePack-CSharp).
...
#1 to #3 are extremely difficult to resolve &ndash; e.g. previously we've managed to implement most of this via ASP.NET Core + WebSockets, but:

- ASP.NET Core controller pipeline is quite inefficient &ndash; it is at least 10x slower than gRPC or SignalR
...
Besides powering Fusion's client compute services (formerly &ndash; replica services), it also allows you to:

- Route calls to different servers based on call parameters &ndash; in other words, run a mesh of services & route calls to them transparently. See https://github.com/ActualLab/Fusion/tree/master/samples/MultiServerRpcApp
- Use server-side call routers &ndash; a service wrappers routing calls to either a local service implementation or an RPC client. This model allows any of your service shards on the server side consume data from any shard (including itself), but w/o triggering any RPC at all for the local data. Unfortunately, there is no sample for this yet.
