...
- Direction: mirror C# — in `addDependency`, return unless `this._state === Computing`; if `dependency` is Invalidated, `this.invalidate()` instead of adding the edge.
...
- Direction: register the computed at creation time, before running the impl, and make `register` invalidate a displaced non-invalidated target like C# (`ComputedRegistry.cs:120-152`); or track in-flight computeds per key in `ComputeFunction` and route key-based invalidations to them.
...
- Direction: pass the child `AsyncContext` to the impl (appended as last arg, matching the existing strip convention) so method bodies can thread it; in Node, back `AsyncContext` with `AsyncLocalStorage`; at minimum detect "nested compute call with no ambient context while a computation is in flight" and warn.
...
- Direction: read arg data right after the 4-byte prefix, parse/skip headers after it; add a cross-format test with a synthetic 1-header message.
...
- Direction: replace with `void computed.whenInvalidated().then(...)` — TS `whenInvalidated()` already resolves immediately for invalidated computeds, exactly mirroring `ProcessStage2`.
...
Mirror the invalidation-audit flow: (1) an adversarial verification pass over the *confirmed* items above (each was produced by a single reviewer; the spot-checked sample held, but the rest deserves the same treatment before implementation); (2) a companion `ts-port-fixes.md` recording the agreed course of action per item, including explicit won't-fix decisions for the deliberate deviations (F6's reconnect simplification, K3's AsyncContext limitation, S16's lazy renewal).
